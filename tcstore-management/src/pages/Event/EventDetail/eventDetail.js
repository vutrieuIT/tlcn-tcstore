...
const EventDetail = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      await axiosClient
        .post(
          "/event/" + id + "/template_feedback/" + template_uid + "/question",
          formatData
        )
        .then((response) => {
          if (response === undefined) {
            ...
            notification["success"]({
              message: `Notification`,
              description: "Successfully Template create",
            });
            setLoading(false);
          }
        });
  ...
  const handleAttendanceScore = async (values) => {
    ...
    try {
      await axiosClient
        .patch("/event/" + uid + "/approve_event", event)
        .then((response) => {
          if (response === undefined) {
            notification["error"]({
              message: `Notification`,
            ...
            notification["success"]({
              message: `Notification`,
              description: "Successfully Approve Event",
            });
            setLoading(false);
            setVisibleUser(false);
            handleListEvent();
      ...
      try {
        await eventApi.generateEvent(uid).then((event) => {
          if (event === undefined) {
            notification["error"]({
              message: `Notification`,
              description: "Generate link meeting to the event failed",
            });
          } else {
            notification["success"]({
              message: `Notification`,
              description: "Successfully generate link meeting to the event",
            });
            handleListEvent();
          }
        });
    ...
    try {
      const response = await eventApi.createQRCode(id).then((response) => {
        if (response === undefined) {
          notification["error"]({
            message: `Notification`,
            description: "QR Code create failed",
          });
          setLoading(false);
        } else {
          notification["success"]({
            message: `Thông báo`,
            description: "Successfully QR Code",
          });
          setLoading(false);
          handleListEvent();
        }
      ...
      <Spin spinning={loading}>
        <Tabs defaultActiveKey="1">
          <TabPane tab="Event Details" key="1">
                  ...
                  <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                    <Col>
                      <div className="event-detail__image">
                        <img
                          className="event-detail__image-one"
                      ...
                      <div className="event-detail__content">
                        <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                          <Col style={{ paddingRight: 0 }}>
                            <Title
                              style={{ marginBottom: 1, color: "#d4380d" }}
                              level={5}
                            >
                            ...
                            >
                              ...
                              {DateTime(eventDetail.start_at, DATE_TIME_FORMAT)}{" "}
                              - {DateTime(eventDetail.end_at, DATE_TIME_FORMAT)}
                            </p>
                            <p style={{ padding: 0, fontSize: 14, margin: 1 }}>
                              <TeamOutlined /> Number Of Participants:{" "}
                              <Tag color="blue">
                                {eventDetail.count_join_event}/
                                {eventDetail.size}
                              </Tag>
                            </p>
                            {eventDetail.status === "accept" ? (
                              <p style={{ padding: 0, fontSize: 14 }}>
                        ...
                        <Row>
                          {!eventDetail.creator_event?.is_creator &&
                          eventDetail.status === "pending" &&
                          role !== "creator" ? (
                            <Popconfirm
                          ...
                          {eventDetail.creator_event?.is_creator === true &&
                          eventDetail.creator_event?.is_qr_code === true ? (
                            <Button
                              shape="round"
                              icon={<QrcodeOutlined />}
                            ...
                            <Popconfirm
                              ...
                              onConfirm={() =>
                                handleCreateQRCode(eventDetail.uid)
                              }
                              okText="Yes"
                              cancelText="No"
                            >
                          ...
                          role !== "creator" ? (
                            <Popconfirm
                              ...
                              onConfirm={() =>
                                handleCancelEvent(eventDetail.uid)
                              }
                              okText="Yes"
                              cancelText="No"
                            >
                          ...
                          {eventDetail.calendar.meeting_url === false &&
                          eventDetail.status === "accept" ? (
                            <Button
                              shape="round"
                              icon={<AppstoreAddOutlined />}
                              onClick={() => showConfirm(eventDetail.uid)}
                            >
                              {"General Link"}
                            </Button>
                          ) : eventDetail.is_online === true &&
                ...
                <Form.Item
                  ...
                  rules={[
                    ...
                    },
                    {
                      ...
                      pattern: /^[-+]?\d+$/,
                      message:
                        "Number of score must be integer and contain just number!",
                    },
                  ]}
              ...
              <Row>
                {eventDetail.type_event?.map((item, index) => {
                  return (
                    <Tag
                      key={index}
                      style={{ marginRight: 5, marginTop: 0, fontSize: 13 }}
                    >
                      {item.toUpperCase()}
                    </Tag>
                  );
            ...
            <Card className="event-detail">
              <LoadScript googleMapsApiKey="AIzaSyDWFEQkm-VD4QPVVYKgFvsYUOFE1MjID5w">
                <GoogleMap
                  mapContainerStyle={containerStyle}
                  center={
                    eventDetail.location === "03 Quang Trung, Đà Nẵng"
                      ? centerQT
                      : eventDetail.location === "245 Nguyễn Văn Linh, Đà Nẵng"
                      ? center
                      : centerHK
                  }
                  zoom={10}
          ...
          <TabPane tab="Evaluation Form" key="2">
            <Row justify="center">
              <div
                className="event-detail-content-evaluation background-eval"
                style={{
                  marginTop: 10,
          ...
          <TabPane tab="Event Rating Statistics" key="3">
            <Row justify="center">
              ...
              >
                ...
                >
                  <strong>Review Collection Form</strong>
                </p>
              </div>
            </Row>
            <Row justify="center">
