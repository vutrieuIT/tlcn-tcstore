...
import { Spin, Space, Card, Table, BackTop, Tag, Breadcrumb, PageHeader, Row, Col, Modal, Form, Button, Select, notification, Alert, Statistic, Steps, message, Input, Result } from 'antd';
...
const OrganizingEventDetails = () => {
      ...
      render: (is_attendance, record) => (
        <Space size="middle">
          {record.is_cancel === true ? (
            ...
            <Tag color="blue">Have Attended</Tag>
          )}
        </Space>
      ),
    },
    ...
    try {
      await axiosClient
        .post(
          "/event/" +
            id +
            "/take_part_in_event/attendance/?email=" +
            values.email
        )
        .then((response) => {
          if (response === undefined) {
            notification["error"]({
              ...
              description: "Enroll users to the event failed",
            });
          } else if (
            response.message === "Errors::ExceptionHandler::InvalidAction"
          ) {
            notification["error"]({
              message: `Notification`,
              description: " User participated in the event",
            });
          } else {
            notification["success"]({
              message: `Notification`,
              description: "Successfully enroll users to the event",
            });
            getListEvent();
          }
        });
  ...
  const handleOkExportExcel = async () => {
    if (
      formExportExcel.getFieldValue("sheet_name") == null &&
      formExportExcel.getFieldValue("spreadsheet_key") == null
    ) {
      ...
      try {
        ...
        await axiosClient
          .post(
            "/event/" + id + "/take_part_in_event/export_list_attendance",
            value
          )
          .then((response) => {
            if (response === undefined) {
              notification["error"]({
                message: `Notification`,
                description: "Export google sheet failed",
              });
            } else if (
              response.message ===
              ...
              notification["success"]({
                message: `Notification`,
                description: "Successfully export google sheet",
              });
              getListEvent();
  ...
  const handleAttendanceUser = async (email) => {
    ...
    try {
      await axiosClient
        .post("/event/" + id + "/take_part_in_event/attendance/?email=" + email)
        .then((response) => {
          if (response === undefined) {
            notification["error"]({
              message: `Notification`,
              description: "Insert users to the event failed",
            });
          } else if (
            response.message === "Errors::ExceptionHandler::InvalidAction"
          ) {
            ...
            notification["success"]({
              message: `Notification`,
              description: "Successfully Insert users to the event",
            });
            getListEvent();
          }
        });
            ...
            <div className="steps-content">
              <div>
                {steps[current].title === "First" ? (
                  <InfiniteScroll dataLength={500} height={450}>
                    <Form
          ...
          <div id="organizing_event_details">
            <div id="organizing_event_details_container">
              <Card title="Students Participate In The Event" bordered={false}>
                <Table
                  columns={columns}
                  dataSource={eventOrganizingDetail}
                  pagination={{
                    onChange(current) {
                      window.scroll(0, 0);
                      setPage(current);
